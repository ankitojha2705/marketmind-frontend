...
async def trigger_schedule(
    ...
    """
    Trigger processing for all schedules that are ready
    """
    try:
        results = await ScheduleService.trigger_schedule(db=db)
        message = (
            "No schedules ready to process"
            if not results
            else f"Processed {len(results)} schedule(s)"
        )
        return {
            "status": "success",
            "message": message,
